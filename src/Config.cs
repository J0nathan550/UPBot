...
public partial class Configs
{
    ...
    public static readonly Dictionary<ulong, List<TagBase>> Tags = [];
    public static readonly Dictionary<ulong, List<string>> SpamLinks = [];
    public static readonly Dictionary<ulong, List<string>> WhiteListLinks = [];

    public static readonly Dictionary<ulong, TempSetRole> TempRoleSelected = [];
    public static string WeatherAPIKey;

    public static readonly Regex emjSnowflakeRE = MyRegex();
    ...
    internal static void LoadParams()
    {
        try
        {
            ...
            if (allAdminRoles != null)
            {
                ...
                {
                    ulong gid = r.Guild;
                    if (!AdminRoles.ContainsKey(gid)) AdminRoles[gid] = [];
            ...
            if (allTags != null)
            {
                ...
                {
                    ulong gid = t.Guild;
                    if (!Tags.ContainsKey(gid)) Tags[gid] = [];
                    Tags[gid].Add(t);
                }
            }
            ...
            List<SpamLink> links = Database.GetAll<SpamLink>();
            if (links != null)
            {
                foreach (var sl in links)
                {
                    if (sl.whitelist)
                    {
                        if (!WhiteListLinks.ContainsKey(sl.Guild)) WhiteListLinks[sl.Guild] = [];
                        WhiteListLinks[sl.Guild].Add(sl.link);
                    }
                    else
                    {
                        if (!SpamLinks.ContainsKey(sl.Guild)) SpamLinks[sl.Guild] = [];
            ...
            // Get WeatherAPIKey
            var WeatherAPIKeys = Database.GetAll<WeatherAPIKey>();
            if (WeatherAPIKeys != null && WeatherAPIKeys.Count > 0)
            {
                WeatherAPIKey = WeatherAPIKeys[0].WeatherApiKey;
            }
            ...
            foreach (var g in Guilds.Keys)
            {
                ...
                if (!AdminRoles.ContainsKey(g)) AdminRoles[g] = [];
                SpamProtections.TryAdd(g, null);
                if (!Tags.ContainsKey(g)) Tags[g] = [];
                TempRoleSelected.TryAdd(g, null);
                if (!SpamLinks.ContainsKey(g)) SpamLinks[g] = [];
                if (!WhiteListLinks.ContainsKey(g)) WhiteListLinks[g] = [];
            }
    ...
    {
        if (!AdminRoles.TryGetValue(gid, out List<ulong> value) || value.Count == 0) return "Admins";
        string res = "";
    ...
    internal static void SetWeatherAPIKey(string key)
    {
        var keys = Database.GetAll<WeatherAPIKey>();
        if (keys != null && keys.Count > 0)
        { // Remove the previous
            Database.Delete(keys[0]);
        }
        Database.Add(new WeatherAPIKey(key)); // Add the new one
        WeatherAPIKey = key;
    }

    [GeneratedRegex(@"<:[a-z0-9_]+:([0-9]+)>", RegexOptions.IgnoreCase | RegexOptions.Compiled, "ru-RU")]
