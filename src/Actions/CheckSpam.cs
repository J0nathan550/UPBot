...
/// author: CPU
/// </summary>
namespace UPBot.src.Actions;

public partial class CheckSpam
{
    private static readonly Regex linkRE = MyRegex();
...
*/
    ...
    private static async Task CheckMessage(DiscordGuild guild, DiscordUser author, DiscordMessage message)
    {
        ...
        try
        {
            if (!Configs.SpamProtections.TryGetValue(guild.Id, out SpamProtection value)) return;
            SpamProtection sp = value;
            if (sp == null) return;
