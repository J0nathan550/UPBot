...
public class Database
{
    ...
    private const string DbName = "BotDb";
    private static Dictionary<Type, EntityDef> entities;
    ...
    public static void InitDb(List<Type> tables)
    {
        try
        {
            ...
            SQLiteCommand cmd = new("SELECT name FROM sqlite_schema WHERE type = 'table'", connection);
    ...
    public static void AddTable(Type t)
    {
        ...
        EntityDef ed = new() { type = t };
        List<FieldInfo> keygens = [];
        foreach (FieldInfo field in t.GetFields())
        {
            bool blob = false;
            bool ignore = false;
    ...
    {
        SQLiteCommand cmd = new("SELECT count(*) FROM " + typeof(T), connection);
    ...
    public static void Add<T>(T val)
    {
        try
        {
            ...
            SQLiteCommand cmd = new(ed.count, connection);
            AddKeyParams(ed, cmd, val);

            // Do we have our value?
            if (Convert.ToInt32(cmd.ExecuteScalar()) > 0)
            { // Yes -> Update
                SQLiteCommand update = new(ed.update, connection);
                foreach (FieldInfo field in t.GetFields())
                {
            ...
            { // No - Insert
                SQLiteCommand insert = new(ed.insert, connection);
                foreach (FieldInfo field in t.GetFields())
                {
    ...
    public static void Delete<T>(T val)
    {
        try
        {
            ...
            SQLiteCommand cmd = new(ed.delete, connection);
            AddKeyParams(ed, cmd, val);
            cmd.ExecuteNonQuery();
        }
        catch (Exception ex)
    ...
    public static T GetByKey<T>(params object[] keys)
    {
        ...
        {
            EntityDef ed = entities[typeof(T)];
            SQLiteCommand cmd = new(ed.selectOne, connection);
            if (ed.keys.Length != keys.Length) throw new Exception("Inconsistent number of keys for: " + typeof(T).FullName);
            int num = 0;
            foreach (var key in ed.keys)
            {
    ...
    public static List<T> GetAll<T>()
    {
        try
        {
            Type t = typeof(T);
            EntityDef ed = entities[t];
            SQLiteCommand cmd = new(ed.select + ";", connection);
            SQLiteDataReader reader = cmd.ExecuteReader();
            List<T> res = [];
            while (reader.Read())
            {
    ...
    private class EntityDef
    {
        ...
        public Dictionary<string, ColDef> fields = [];
        public string count;
