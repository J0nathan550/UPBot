...
public class SlashTimezone : ApplicationCommandModule
{
    ...
    public static async Task TZTimeCommand(InteractionContext ctx, [Option("timezone", "Timezone to check the local time")] string tz)
    {
    ...
    public static async Task TZTimeGetCommand(InteractionContext ctx, [Option("user", "The user for the time")] DiscordUser user)
    {
        Utils.LogUserCommand(ctx);
    ...
    public static async Task TZSetCommand(InteractionContext ctx, [Option("user", "The user to set the timezone")] DiscordUser user, [Option("timezone", "Timezone to check the local time")] string tz)
    {
        try
        {
            DiscordMember member = await ctx.Guild.GetMemberAsync(user.Id);
    ...
    public static async Task TZListCommand(InteractionContext ctx)
    {
        try
        {
            string res = "```\n";
            ...
            {
                if (!count.TryGetValue(t.TimeZoneName, out int value)) count[t.TimeZoneName] = 1;
                else count[t.TimeZoneName] = ++value;
            }
            string bads = "";
            int numbads = 0;
    ...
    private static int GetTZScore(string src, string dst)
    {
    ...
    private static List<RankedTimezone> CheckProx(string inp)
    {
    ...
    private static List<RankedTimezone> GetTimezone(string input, out TimeZoneInfo res)
    {
