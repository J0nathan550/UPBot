    ...
    private static async Task<string> CalculateUserMentions(InteractionContext ctx)
    {
        ...
        foreach (var m in msgs)
        {
            ...
            foreach (var r in mens)
            {
                ...
                if (count.TryGetValue(snowflake, out int value)) count[snowflake] = ++value;
                else count[snowflake] = 1;

                if (!askers.TryGetValue(m.Author.Id, out value)) askers[m.Author.Id] = 1;
                else askers[m.Author.Id] = ++value;
            }
        }
        List<KeyValuePair<string, int>> list = [];
        foreach (var k in count.Keys) list.Add(new KeyValuePair<string, int>(k, count[k]));
        ...
        List<KeyValuePair<string, int>> listask = [];
